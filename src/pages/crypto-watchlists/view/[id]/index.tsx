...
import { getCryptoWatchlistById } from 'apiSdk/crypto-watchlists';
import { CryptoWatchlistInterface } from 'interfaces/crypto-watchlist';

function CryptoWatchlistViewPage() {
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Crypto Watchlist Details',
              isCurrent: true,
            },
          ]}
        />
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                <Box>
                  ...
                  >
                    Crypto Watchlist Details
                  </Text>
                </Box>
                {hasAccess('crypto_watchlist', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/crypto-watchlists/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('crypto_market', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Crypto Market"
                    text={
                      <Link as={NextLink} href={`/crypto-markets/view/${data?.crypto_market?.id}`}>
                        {data?.crypto_market?.name}
                      </Link>
                    }
                  />
                )}
