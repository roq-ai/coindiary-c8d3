...
import { getCryptoWatchlists } from 'apiSdk/crypto-watchlists';
...
import { getCryptoNews } from 'apiSdk/crypto-news';
import { CryptoNewsInterface } from 'interfaces/crypto-news';

import { CryptoWatchlistListPage as ShowCaseList } from 'pages/crypto-watchlists';

function DashboardPage() {
  const { session } = useSession();

  const cryptoWatchlistStat = useSWR<PaginatedInterface<CryptoWatchlistInterface>>(
    () => `/crypto-watchlists`,
    () => getCryptoWatchlists({ limit: 1, offset: 0 }),
  );

  const cryptoMarketStat = useSWR<PaginatedInterface<CryptoMarketInterface>>(
    () => `/crypto-markets`,
    () => getCryptoMarkets({ limit: 1, offset: 0 }),
  );

  const cryptoNewsStat = useSWR<PaginatedInterface<CryptoNewsInterface>>(
    () => `/crypto-news`,
    () => getCryptoNews({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Crypto Watchlists',
      result: cryptoWatchlistStat?.data?.totalCount,
      isLoading: cryptoWatchlistStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Crypto Markets',
      result: cryptoMarketStat?.data?.totalCount,
      isLoading: cryptoMarketStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Crypto News',
      result: cryptoNewsStat?.data?.totalCount,
      isLoading: cryptoNewsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Crypto Watchlists
                </Text>
                <NextLink href={`crypto-watchlists`} passHref legacyBehavior>
                  <Button
